...
@if ( count($users) > 0 )
	...
	<tr>
		...
		@endforeach
		<th width="10%" class="hidden-xs">Ranking</th>
...
@foreach ( $users as $member )
	...
	@endforeach
		...
		#{{ $member->victory_rank->rank }} - <strong>{{ number_format($member->victory_rank->score) }}</strong>
		@else
		&nbsp;
		@endif
		</td>
		<td class="date">{{ Helpers::local_date('M j, Y', $member->created_at) }}</td>
